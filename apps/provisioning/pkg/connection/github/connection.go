...
// TokenState reports the state of the stored JWT. A token that does not parse
// with the private key or was issued for another appID is not usable.
func (c *Connection) TokenState() (bool, time.Time) {
	claims, err := parseJWTToken(c.secrets.Token, c.secrets.PrivateKey)
	if err != nil || claims.Issuer != c.cfg.AppID() {
		return false, time.Time{}
	}
	if claims.ExpiresAt != nil {
		return true, claims.ExpiresAt.Time
	}
