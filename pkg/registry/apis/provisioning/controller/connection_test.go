	...
	testCases := []struct {
		...
		},
		{
			...
			setupMocks: func() (*mockConnectionLister, *MockConnectionHealthChecker, *MockConnectionStatusPatcher, *connection.MockFactory) {
				...
				mockTokenConnection.EXPECT().TokenState().Return(true, time.Now().Add(2*time.Minute))
		...
		},
		{
			...
			setupMocks: func() (*mockConnectionLister, *MockConnectionHealthChecker, *MockConnectionStatusPatcher, *connection.MockFactory) {
				...
				mockTokenConnection.EXPECT().TokenState().Return(true, time.Now().Add(2*time.Hour))
				mockHealthChecker.EXPECT().RefreshHealthWithPatchOps(mock.Anything, mock.Anything).
					Return(ConnectionHealthResultWithPatchOps{
						TestResults:  testResults,
						HealthStatus: healthStatus,
		...
		},
		{
			...
			setupMocks: func() (*mockConnectionLister, *MockConnectionHealthChecker, *MockConnectionStatusPatcher, *connection.MockFactory) {
				...
				mockTokenConnection.EXPECT().TokenState().Return(true, time.Now().Add(15*time.Minute))
				mockHealthChecker.EXPECT().RefreshHealthWithPatchOps(mock.Anything, mock.Anything).
					Return(ConnectionHealthResultWithPatchOps{
						TestResults:  testResults,
						HealthStatus: healthStatus,
		...
		},
		{
			...
			setupMocks: func() (*mockConnectionLister, *MockConnectionHealthChecker, *MockConnectionStatusPatcher, *connection.MockFactory) {
				...
				mockTokenConnection.EXPECT().TokenState().Return(true, time.Now().Add(9*time.Minute))
		...
		},
		{
			...
			setupMocks: func() (*mockConnectionLister, *MockConnectionHealthChecker, *MockConnectionStatusPatcher, *connection.MockFactory) {
				...
				mockTokenConnection.EXPECT().TokenState().Return(false, time.Time{})
				mockTokenConnection.EXPECT().GenerateConnectionToken(mock.Anything).Return(common.RawSecureValue("new-token"), nil)
				mockHealthChecker.EXPECT().RefreshHealthWithPatchOps(mock.Anything, mock.Anything).
					Return(ConnectionHealthResultWithPatchOps{
						TestResults:  testResults,
